...
function KtxImageChecks()
    -- load official ktx testimages and check loaded okay
    function KtxTestImageLoadAndSimpleCheck(prefix, fname, expected_width, expected_height, expected_depth, expected_slices, expected_cubemap, expected_format)
        local test, okay = image.load( prefix .."/testimages/" .. fname)
        if okay ~= true then
            error("Could not load " .. prefix .. "/testimages/" .. fname)
        end


        if test:width() ~= expected_width then
            print(prefix .."/testimages/" .. fname .. " load failed with width = " .. test:width() .. " it should be " .. expected_width)
        end

        if test:height() ~= expected_height then
            print(prefix .."/testimages/" .. fname .. " load failed with height = " .. test:height() .. " it should be " .. expected_height)
        end
        if test:depth() ~= expected_depth then
            print(prefix .."/testimages/" .. fname .. " load failed with depth = " .. test:depth() .. " it should be " .. expected_depth)
        end
        if test:slices() ~= expected_slices then
            print(prefix .."/testimages/" .. fname .. " load failed with slices = " .. test:slices() .. " it should be " .. expected_slices)
        end
        if test:isCubemap() ~= expected_cubemap then
            print(prefix .."/testimages/" .. fname .. " load failed as cubemap = ", test:isCubemap(), " it should be ", expected_cubemap)
        end

        if test:format() ~= expected_format then
            print(prefix .."/testimages/" .. fname .. " load failed with format = " .. test:format() .. " it should be " .. expected_format)
        end
        return test
    end

    for i,tex in ipairs(ktx_tests) do
        local test = KtxTestImageLoadAndSimpleCheck("input", tex[1], tex[2], tex[3], tex[4], tex[5], tex[6], tex[7])
        test:saveAsKTX("artifacts/testimages/"..tex[1])
    end
end

function PvrBadCheck() 
